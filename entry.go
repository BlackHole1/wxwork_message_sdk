...
type Wx struct {
	...
	RegistryUserHandle map[string]func(textContent string) (content string, err error)
	Delimiters         []string
}
...
func Create(token string, corpId string, encodingAesKey string) func(path string, port string, delimiters []string) *Wx {
	return func(path string, port string, delimiters []string) *Wx {
		...
		return &Wx{
			...
			RegistryUserHandle: make(map[string]func(textContent string) (content string, err error)),
			WxCrypt:            wxCrypt,
			Path:               path,
			Port:               port,
...
// 注册消息
func (w *Wx) Registry(handle func(textContent string) (content string, err error), levels ...string) {
	w.RegistryHandle[strings.ToLower(strings.Join(levels, " "))] = handle
}

// 注册发送人
func (w *Wx) RegistryUser(handle func(textContent string) (content string, err error), fromUsername string) {
	w.RegistryUserHandle[fromUsername] = handle
}
